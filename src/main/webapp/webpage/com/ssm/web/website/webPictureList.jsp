...
<script type="text/javascript">
	...
	function to_page(pn){
		...
		$.ajax({
			url:"${APP_PATH}/webpicsAjax",
			data:"pn="+pn+"&pageNum="+pageNum+"&linkId="+'${linkId}'+'&'+query,
			type:"POST",
			success:function(result){
				//console.log(result);
				//清空数据
	...
	function build_data_table(result){
		...
		$.each(data,function(index,item){
			...
			var urlName = $("<td hidden=\"hidden\"></td>").append(item.webLinkBase.linkName);
			var picNameTmp = item.picName;
			picNameTmp = picNameTmp!=null? picNameTmp.replace(/([^\u0000-\u00FF])+/g, function ($) { return "****";/* escape($);  */}):"";
			var picName = $("<td></td>").append(picNameTmp);
			var oldName = item.picOldname;
			oldName = oldName!=null? oldName.replace(/([^\u0000-\u00FF])+/g, function ($) { return "****";/* escape($);  */}):"";
			//alert(oldName);
			...
			picPath = picPath!=null?picPath.replace(/([^\u0000-\u00FF])+/g, function ($) { return "****";/* escape($);  */}):"暂未下载";
			...
			var viewpicBtn = $("<button></button>").addClass("btn btn-success btn-xs edit_btn")
						.append($("<span></span>").addClass("glyphico glyphicon-")).append("预览");
	...
	function viewpic_page_handle(id){
		window.open("${APP_PATH}/viewWebpic?id="+id, "图片预览");
	}
	
	function mark_handle(id,result){
		var pageInfo = result.extend.pageInfo;
		$.ajax({
			url:"${APP_PATH}/markWebpic",
			data:"id="+id,
			type:"POST",
			success:function(result){
				if(result.code==200){
	...
	$("#pic_view_all").click(function(){
		...
		window.open("${APP_PATH}/viewWebPicList?ids="+ids, "多图预览");
	});
	...
	$("#pic_view_all_roll").click(function(){
		...
		window.open("${APP_PATH}/viewWebpicRollList?ids="+ids, "相册预览");
	});
